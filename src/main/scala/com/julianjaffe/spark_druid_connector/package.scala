...
import com.julianjaffe.spark_druid_connector.model.DeepStorageConfig
import com.julianjaffe.spark_druid_connector.partitioners.{HashBasedNumberedPartitioner,
  NumberedPartitioner, SingleDimensionPartitioner}
import com.julianjaffe.spark_druid_connector.v2.DruidDataSourceV2ShortName
...
package object spark_druid_connector { // scalastyle:ignore package.object.name
  private[spark_druid_connector] val MAPPER: ObjectMapper = new DefaultObjectMapper()

  private[spark_druid_connector] val baseInjectableValues: InjectableValues.Std =
  ...
  implicit class DruidDataFrameWriter[T](private[spark_druid_connector] val writer: DataFrameWriter[T]) {
    def druid(): Unit = {
  ...
  implicit class DruidDataFrameReader(private[spark_druid_connector] val reader: DataFrameReader) {
    def druid(): DataFrame = {
  ...
  implicit class DruidDataFrame(private[spark_druid_connector] val df: DataFrame) extends Serializable {
