...
import org.apache.druid.query.filter.{AndDimFilter, BoundDimFilter, DimFilter, FalseDimFilter, InDimFilter, NotDimFilter, OrDimFilter, RegexDimFilter, SelectorDimFilter}
...
import org.apache.spark.sql.types.{ArrayType, DataType, DoubleType, FloatType, IntegerType, LongType, StringType, StructType, TimestampType}
...
object FilterUtils {
  ...
  /**
    ...
    */
  private[spark_druid_connector] def isSupportedFilter(
                                        filter: Filter,
                                        schema: StructType,
  ...
  /**
    ...
    *         epoch millis.
    */
  private[spark_druid_connector] def decomposeTimeFilters(filter: Filter): Seq[(Bound, Long)] = {
    filter match {
      case And(left, right) =>
  ...
  private[spark_druid_connector] sealed trait Bound
