...
import org.apache.druid.query.filter.{DimFilter, SelectorDimFilter}
...
class DruidInputPartitionReader(
  ...
  private val firehose: IngestSegmentFirehose = DruidInputPartitionReader.makeFirehose(
    ...
    new SelectorDimFilter("added","18",null,null),//MAPPER.readValue[DimFilter](filter.get, new TypeReference[DimFilter] {}), //一开始是filter.orNull，由于spark sql 序列化问题没法,没法使用过滤器，谓词下推
